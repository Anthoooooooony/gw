...
//
// v0：纯透传；仅用来验证 ANTHROPIC_BASE_URL 重定向链路。
// 后续在 RoundTrip 位置插入 tool_result 去重等上下文裁剪逻辑。
package apiproxy
...
// Transformer 的引用也被 Server 持有，以便调用方通过 Stats() 读取去重观测数据。
type Server struct {
	ln          net.Listener
	httpSrv     *http.Server
	addr        string
	transformer *dedup.Transformer
}
...
func Start(logger Logger) (*Server, error) {
	...
	transformer := dedup.NewTransformer(logger)
...
// Stats 返回 dedup Transformer 的累积观测数据（非快照：继续被后续请求更新）。
func (s *Server) Stats() *dedup.Stats { return s.transformer.Stats() }
...
// 类型别名自 dedup.Logger，避免两处声明同形接口漂移：apiproxy 依赖 dedup，
// 复用 dedup 的声明不破坏 dedup 的独立可用性（dedup 是可单独使用的上下文压缩库）。
type Logger = dedup.Logger
