...
**Goal:** 搭建 gw 首版 release 管道——CHANGELOG、bump 脚本、GoReleaser 配置、release GitHub Action、dev 分支设立，并发布 `v0.1.0`。

**Architecture:** 两干分支模型（dev + master），SemVer 版本号，GoReleaser split/merge 多平台原生 CGO 构建。bump.sh 读 git 历史生成 CHANGELOG + tag + push；tag push 触发 release.yml 在三台 runner（ubuntu/macos-13/macos-latest）并行构建，合并产出 GitHub Release + 3 个 tar.gz + checksums.txt。
...
## Task 8: 建立 dev 分支 + GitHub 默认分支切换

**Files:**
- （无文件改动，仅 git + GitHub 设置）

- [ ] **Step 1: 推送当前 master 改动**

```bash
git push origin master
```

Expected: master 推到远端，至少包含 Task 1-7 的所有 commit。

- [ ] **Step 2: 基于 master 创建 dev 分支并推送**
...
Expected: 远端出现 `refs/heads/dev`，本地 dev 追踪 origin/dev。

- [ ] **Step 3: GitHub 网页上将 dev 设为 default branch**

手动操作（gh CLI 也能做到，但网页更清晰）：

```bash
# 等效 gh CLI 命令：
gh api --method PATCH repos/Anthoooooooony/gw \
  -f default_branch=dev
```

验证：
```bash
gh repo view Anthoooooooony/gw --json defaultBranchRef -q .defaultBranchRef.name
```

Expected: `dev`

- [ ] **Step 4: 回到 master 分支（后续 Task 9 还要用）**
...
git checkout dev
git pull origin dev
git merge master --no-ff -m "chore: merge v0.1.0 release to dev"
git push origin dev
git checkout master
```

Expected：dev 现在包含 `v0.1.0` tag 的 CHANGELOG commit。
...
- [ ] **Step 4（可选）: 开 PR 把 dev 上的 docs 同步到 master**
...
验证新 workflow：PR 的 base 字段应该自动填 `master`（因为 dev 现在是 default，`--base master` 是显式指定），head 是 dev。

Expected: 这是第一个走新 dev → master 流程的 PR，验证默认 base 已切到 dev。
