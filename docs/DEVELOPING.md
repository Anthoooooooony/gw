...
│       └── dedup/                 # tool_result 内容去重（同签名 tool_use 历史替换为占位符）
...
当 `gw claude` 行为异常（请求没进代理、去重替换没生效等）时：

- **verbose 模式打逐请求日志**：`gw -v claude` 会在 stderr 打每条 `/v1/messages` 的 `dedup: 替换 N 条 tool_result` / `dedup: 解析失败，透传` 信息
- **每次退出都会打出统计摘要**（非 verbose 也会）：`gw: dedup: N 请求 / 扫 M tool_use / 替换 K tool_result / 节省 B 字节`；0 请求时静默
