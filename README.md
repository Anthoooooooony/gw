...
```bash
gw summary           # 累计：多少命令、省了多少 token、按命令排 Top N（gain 仍作为别名保留）
...
│   ├── summary.go                  # gw summary — Token 节省统计（gain 作为 alias 保留）
...
| `gw summary` (alias: `gw gain`) | 查看 token 节省统计；顺便按 `GW_DB_MAX_BYTES` 阈值裁剪 DB |
