...
日常贡献流程（分支命名、Commit 约定、CHANGELOG 维护、release 步骤、测试层级）见 [`CONTRIBUTING.md`](./CONTRIBUTING.md)。Claude Code 的项目级开发约定、代码规范与关键不变式见 [`CLAUDE.md`](./CLAUDE.md)。架构 / 公开接口 / 产品边界相关的历史决策（含否决方案）追溯在 [`docs/DECISIONS.md`](./docs/DECISIONS.md)（ADR-lite 格式）。
