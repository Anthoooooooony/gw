...
用户口述需求时**先摸底再起草**，避免与既有 issue / 已实现重复：

1. `gh issue list -R Anthoooooooony/gw --state all --limit 20` 看是否已有相近议题
2. 必要时 `grep` / 读相关代码，确认 issue 是"新增"还是"扩展现有"——措辞与优先级判断都靠这步
3. 给用户预览：**Title** + **Body**（"现状 / 问题 / 期望" 必有，"待讨论" 可选）+ **确认事项**（label / repo）
4. 用户确认后 HEREDOC 提交：

   ```bash
   gh issue create -R Anthoooooooony/gw \
     --title "<CC前缀>: <短描述>" \
     --label "priority:near,bug" \
     --body "$(cat <<'EOF'
   ## 现状
   ...
   EOF
...
- **Title 用 Conventional Commits 前缀**，与未来 PR title 对齐
- **Label 必须同时打 priority + 类型两个维度**（详见 [`CONTRIBUTING.md`](./CONTRIBUTING.md#issue-规范)）：
  - priority 维度：`priority:near` / `priority:mid` / `priority:long` 三选一，未指明默认 `priority:near`
  - 类型维度：`bug` / `feature` 二选一，按 issue 内容判断，模糊时与用户确认
  - 其它 label 走 `gh label list -R Anthoooooooony/gw` 查
- HEREDOC 必须用 `'EOF'`（带单引号），否则 body 里的 `$()` / 反引号会被 shell 展开
- issue 之间默认**解耦**，不互相引用编号——除非确有强依赖
...
- **认知同步**：发现自己引用的接口 / 路径 / 包名与代码现状对不上时，**停下推断**——先 `git log --oneline -20` 看主线变化、再 `grep` / 读文件对齐，再继续答用户；若 CLAUDE.md / `docs/` 与代码冲突，明确向用户报告差异，而非按陈旧记忆继续
...
- **提 PR 后默认自动监听 CI**：`gh pr checks <N> --watch`（可 `run_in_background`），不等用户催；红则按日志定位根因再修，绿则汇报可 merge 状态
